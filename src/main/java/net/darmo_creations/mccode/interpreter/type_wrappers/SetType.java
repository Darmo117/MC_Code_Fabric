...
public class SetType extends TypeBase<MCSet> {
  ...
  @Override
  protected NbtCompound _writeToNBT(final MCSet self) {
  ...
  @Override
  public MCSet readFromNBT(final Scope scope, final NbtCompound tag) {
    MCSet set = new MCSet();
    NbtList tagsList = tag.getList(VALUES_KEY, NbtElement.COMPOUND_TYPE);
    tagsList.forEach(t -> set.add(ProgramManager.getTypeForName(((NbtCompound) t).getString(TypeBase.NAME_KEY)).readFromNBT(scope, (NbtCompound) t)));
    return set;
  }
}
